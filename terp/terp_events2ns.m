function terp_events2ns(dataroot,subject,session)
% terp_events2ns(dataroot,subject,session)
%
...
%dataroot = '/Volumes/curranlab/Data/TERP/eeg/behavioral';
% subject = 'TERP001';
% session = 'session_0';

%% constants

expectedSampleRate = 250;
...
%% set up files
eventsDir = fullfile(dataroot,subject,'events');
sesNum = str2double(strrep(session,'session_',''));

%% load in and process the events
evFile = fullfile(eventsDir,sprintf('events_ses%d.mat',sesNum));
fprintf('Loading %s...\n',evFile);
events = loadEvents(evFile);
...
% get the samplerate
[nsSampleRate,~,~,~] = GetRateAndFormat(fullfile(eegdir,'eeg.noreref'));

% make sure it's this_SR, otherwise throw an error because we're not
% reading params.txt correctly
if nsSampleRate ~= expectedSampleRate
  error('NetStation file samplerate is %.3f, not %.3f Hz as expected. Something is wrong.',nsSampleRate,expectedSampleRate);
end
...
% TODO: prepare NS coding for TERP experiment (codes below are from SOSI)
fprintf('%s IS NOT DONE YET!!!\n',mfilename);
...
for i = 1:length(events)
  ...
  rel_mstime = (events(i).eegoffset / nsSampleRate) * 1000;
  ...
  xcrd = num2str(events(i).xcoord);
  ycrd = num2str(events(i).ycoord);
  ...
  %cellLabels = {subStr,sesStr,listStr,trialStr,typeStr,itemStr,poolStr,pcorStr,xcrdStr,ycrdStr,srpsStr,rec_targStr,scndStr,sturStr,strtStr,src_targStr,srcrStr,srrtStr,scorStr,rknrStr,rkrtStr,rcorStr,artiStr};
  %cellData = {subject,session,list,trial,type,item,pool,pool_correct,xcrd,ycrd,serpos,rec_isTarg,study_cond,study_resp,study_rt,src_isTarg,src_resp,src_rt,src_correct,rkn_resp,rkn_rt,rkn_correct,artifactMS};
  cellLabels = {subStr,sesStr,listStr,trialStr,typeStr,itemStr,srpsStr,xcrdStr,ycrdStr,rec_targStr,rcorStr,src_targStr,srcrStr,srrtStr,scorStr,rknrStr,rkrtStr,rkncStr};
  cellData = {subject,session,list,trial,type,item,serpos,xcrd,ycrd,rec_isTarg,rec_correct,src_isTarg,src_resp,src_rt,src_correct,rkn_resp,rkn_rt,rkn_correct};
