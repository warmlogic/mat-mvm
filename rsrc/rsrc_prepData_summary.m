function rsrc_prepData_summary(rejectArt,saveFiles,dataroot)
% function rsrc_prepData_summary(dataroot)
%
...
% recognition confidence numbers are [6 5 4 3 2 1], sure old to sure new
confRange_rec = [6 5 4 3 2 1];
% source confidence numbers are [1 2 3 4 5 6], sure L to sure R
...
if rejectArt == 0
  tableHeader = {'sub','ses',... % subject info
    ...
    'Item d''','Source d''','Item da','Source da','Item zROC slope','Source zROC slope','Item c','Source c','Item ca','Source ca',... % accuracy
...
elseif rejectArt == 1
  tableHeader = {'sub','ses',... % subject info
    'R targ','R lure','S targ','S lure',... % raw numbers of targs, lures
    'Source d''','Source da','Source zROC slope','Source c','Source ca',... % accuracy
...
%         axis square
        ...
        % c_a from RscorePlus manual p. 13
        %
        % Equation 3.13, page 67 of Macmillan & Creelman, 2005
        item_ca = mean((((-1 * sqrt(2)) * item_z_slope) / ((1 + item_z_slope^2)^(1/2) * (1 + item_z_slope))) * (rec_roc_zhr + rec_roc_zfar));
        source_ca = mean((((-1 * sqrt(2)) * source_z_slope) / ((1 + source_z_slope^2)^(1/2) * (1 + source_z_slope))) * (src_roc_zhr + src_roc_zfar));
...
%         axis square
      ...
      if rejectArt == 0
        tableData = [...
          ...
          item_dp,source_dp,item_da,source_da,item_z_slope,source_z_slope,item_c,source_c,item_ca,source_ca,... % accuracy
      ...
      elseif rejectArt == 1
        tableData = [...
          length(rec_targEv),length(rec_lureEv),length(src_targEv),length(src_lureEv),... % raw numbers of targets, lures
          source_dp,source_da,source_z_slope,source_c,source_ca,... % accuracy
