...
%             data_p2 = reshape(data_p2, dim(1), prod(dim(2:end)));
    ...
    saveFile = fullfile(dirs.saveDirProc,exper.subjects{sub},exper.sesStr{ses},sprintf('RSA_PCA_pow_classif_%s_%s_%dlat_%dfreq_%sAvgT_%sAvgF_%s.mat',eig_criterion,roi_str,size(latencies,1),size(freqs,1),cfg_sel.avgovertime,cfg_sel.avgoverfreq,date));
