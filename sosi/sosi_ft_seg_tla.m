...
exper.nTrials.thresh = 15;

% Subjects with bad behavior
exper.badBehSub = {'SOSI011','SOSI030'};

% exclude subjects with low event counts
[exper] = mm_threshSubs(exper,ana,dirs);
...
for ses = 1:length(exper.sessions)
  for evVal = 1:length(ana.eventValues{1})
    %tic
    fprintf('Running ft_timelockgrandaverage on %s...',ana.eventValues{1}{evVal});
    ga_tla.(ana.eventValues{1}{evVal})(ses) = eval(sprintf('ft_timelockgrandaverage(cfg_ft,%s);',cfg_ana.sub_str.(ana.eventValues{1}{evVal}){ses}));
    fprintf('Done.\n');
    %toc
  end
end
