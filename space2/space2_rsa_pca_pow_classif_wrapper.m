function space2_rsa_pca_pow_classif_wrapper(whichStages)
% space2_rsa_pca_pow_classif_wrapper(whichStages)
%
% This is the first file to run for doing PCA-based RSA on dream.
%
% To run on dream, at the command line type: distmsub2011 space2_rsa_pca_pow_classif_wrapper.m
%
...
for i = STAGES
  ...
  if i == 1
    stageFun{i}(expName,saveDirProc,replaceDataroot,replaceDatatype,gradeCorrect,accurateClassifSelect,date_string,dataTypes,cfg_sel,subjects,sesNames,allROIs,allLats,allFreqs,allSimMethod,allEigCrit,runLocally,timeOut{i});
  %elseif i == 2
...
function stage1(expName,saveDirProc,replaceDataroot,replaceDatatype,gradeCorrect,accurateClassifSelect,date_string,dataTypes,cfg_sel,subjects,sesNames,allROIs,allLats,allFreqs,allSimMethod,allEigCrit,runLocally,timeOut)
        ...
        for f = 1:length(allFreqs)
          ...
          for s = 1:length(allSimMethod)
            ...
            for e = 1:length(allEigCrit)
              ...
              inArg = {saveDirProc,replaceDataroot,replaceDatatype,gradeCorrect,accurateClassifSelect,date_string,dataTypes,cfg_sel,thisSub,thisSes,thisROI,latencies,freqs,sim_method,eig_criterion};
              createTask(job,@space2_rsa_pca_pow_classif_cluster,0,inArg);
            end
          end
        end
      end
      ...
      for f = 1:length(allFreqs)
        ...
        for s = 1:length(allSimMethod)
          ...
          for e = 1:length(allEigCrit)
            ...
            space2_rsa_pca_pow_classif_cluster(saveDirProc,replaceDataroot,replaceDatatype,gradeCorrect,accurateClassifSelect,date_string,dataTypes,cfg_sel,subjects,sesNames,thisROI,latencies,freqs,sim_method,eig_criterion);
          end
        end
      end
    end
