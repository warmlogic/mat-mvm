    ...
    for mc = 1:length(memConds)
      ...
      if strcmp(spacings{sp},'onePres')
        ...
        if strcmp(memConds{mc},'all')
          cfg.conditions = {sprintf('%s%s%s_%s',stimType,memType,'rc',spacings{sp}), sprintf('%s%s%s_%s',stimType,memType,'fo',spacings{sp})};
        else
          cfg.conditions = {sprintf('%s%s%s_%s',stimType,memType,memConds{mc},spacings{sp})};
        end
      ...
      elseif strcmp(spacings{sp},'mass') || strcmp(spacings{sp}(1:4),'spac')
        if strcmp(memConds{mc},'all')
          ...
          cfg.conditions = {sprintf('%s%s%s_%s_%s',stimType,memType,'rc',spacings{sp},oldnew{on}), sprintf('%s%s%s_%s_%s',stimType,memType,'fo',spacings{sp},oldnew{on})};
        else
          cfg.conditions = {sprintf('%s%s%s_%s_%s',stimType,memType,memConds{mc},spacings{sp},oldnew{on})};
        end
      ...
      for er = 1:length(erpComponents)
        for r = 1:length(roi{er})
          ...
          if strcmp(erpComponents{er},'LPC')
            ...
            cfg.latency = [ana.lpcPeak-(ana.lpcPeakWindow/2), ana.lpcPeak+(ana.lpcPeakWindow/2)]; % LPC - around GA peak (space+mass) +/- 100
          ...
          elseif strcmp(erpComponents{er},'N400')
            ...
            cfg.latency = [ana.n400Peak-(ana.n400PeakWindow/2), ana.n400Peak+(ana.n400PeakWindow/2)]; % N400 - around GA peak (space+mass) +/- 50
            %cfg.latency = [n400Peak-0.1 n400Peak+0.1]; % N400 - around GA peak (space+mass) +/- 100
          elseif strcmp(erpComponents{er},'N1')
            cfg.order = 'ascend'; % ascend = negative peaks first
            cfg.latency = [ana.n1Peak-(ana.n1PeakWindow/2), ana.n1Peak+(ana.n1PeakWindow/2)]; % around GA peak (space+mass) +/- 50
    ...
    for on = 1:length(oldnew)
      for mc = 1:length(memConds)
        ...
        for r = 1:length(roi)
          ...
          anovaData(sub,vnCount) = mean(allPeakInfo.(cond_str).(roi_str).subjects.(measure)(sub,1:nPoints));
...
mass_volt = allPeakInfo.(mass_str).(erpComp).subjects.voltage(:,8);
