function space2_rsa_pca_pow_classif_cluster(procDir,replaceDataroot,replaceDatatype,gradeCorrect,accurateClassifSelect,date_string,dataTypes,cfg_sel,subjects_all,sesNames_all,thisROI,latencies,freqs,sim_method,eig_criterion)

% space RSA

% this file gets run by space2_rsa_pca_pow_classif_wrapper.m
...
%     end
    ...
    saveFile = fullfile(dirs.saveDirProc,exper.subjects{sub},exper.sesStr{ses},sprintf('RSA_PCA_pow_%s_%s_%s_%s_%s_%dlat_%s_%sAvgT_%sAvgF_%s.mat',data_str,sim_method,classif_str,eig_criterion,roi_str,size(latencies,1),freq_str,cfg_sel.avgovertime,cfg_sel.avgoverfreq,date_string));
    fprintf('Saving: %s\n',saveFile);
