function mm_ft_rmaov2TFR(cfg_ana,exper,ana,data)
%MM_FT_RMAOV2TFR 2-way RMANOVA on time-frequency data
%   
...
for typ = 1:length(cfg_ana.conditions)
  ...
  fprintf('%.1f--%.1f s, %.1f--%.1f Hz: IV1: %s (%d;%s), IV2: %s (%d;%s)\n',...
    ...
    for c = 1:length(cfg_ana.cond)
      ...
      for sub = 1:length(exper.subjects)
        ...
        if exper.badSub(sub,ses)
          ...
          cfg_ana.freqsel.(cfg_ana.cond{c}) = find(data.(cfg_ana.cond{c}).sub(sub).ses(ses).data.freq >= cfg_ana.frequency(1) & data.(cfg_ana.cond{c}).sub(sub).ses(ses).data.freq <= cfg_ana.frequency(2));
          cfg_ana.values.(cfg_ana.cond{c})(goodSubInd,ses) = mean(mean(mean(data.(cfg_ana.cond{c}).sub(sub).ses(ses).data.(cfg_ana.parameter)(cfg_ana.chansel,cfg_ana.freqsel.(cfg_ana.cond{c}),cfg_ana.timesel.(cfg_ana.cond{c})),1),2),3);
        end
  ...
  % IV1 (ROI)
  fprintf('====================== RMAOV1_mod =========================\n');
  fprintf('%.1f--%.1f s, %.1f--%.1f Hz: IV1: %s (%d;%s), collapsing over %s (%d;%s)\n',...
    cfg_ana.latency(1),cfg_ana.latency(2),...
    cfg_ana.frequency(1),cfg_ana.frequency(2),...
    cfg_ana.IV_names{1},length(cfg_ana.roi),cfg_ana.chan_str,...
