function mm_ft_rmaov33TFR(cfg_ana,exper,ana,data)
%MM_FT_RMAOV33TFR 3-way RMANOVA; 2- & 1-way RMANOVAs collapsing across IVs
%   

if ~isfield(cfg_ana,'parameter')
  error('Must specify cfg_ana.parameter, denoting the type of data (e.g., ''powspctrm'' or ''cohspctrm''');
    ...
    for c = 1:length(cfg_ana.cond)
      ...
      for sub = 1:length(exper.subjects)
        ...
        if exper.badSub(sub,ses)
          ...
          cfg_ana.freqsel.(cfg_ana.types{typ}).(cfg_ana.cond{c}) = find(data.(cfg_ana.cond{c}).sub(sub).ses(ses).data.freq >= cfg_ana.frequency(1) & data.(cfg_ana.cond{c}).sub(sub).ses(ses).data.freq <= cfg_ana.frequency(2));
          cfg_ana.values.(cfg_ana.types{typ}).(cfg_ana.cond{c})(goodSubInd,ses) = mean(mean(mean(data.(cfg_ana.cond{c}).sub(sub).ses(ses).data.(cfg_ana.parameter)(cfg_ana.chansel,cfg_ana.freqsel.(cfg_ana.types{typ}).(cfg_ana.cond{c}),cfg_ana.timesel.(cfg_ana.types{typ}).(cfg_ana.cond{c})),1),2),3);
        end
...
fprintf('====================== RMAOV33_mod =========================\n');
fprintf('%.1f--%.1f s, %.1f--%.1f Hz: IV1: %s (%d;%s), IV2: %s (%d;%s), IV3: %s (%d;%s)\n',...
  cfg_ana.latency(1),cfg_ana.latency(2),...
  cfg_ana.frequency(1),cfg_ana.frequency(2),...
...
% IV1xIV2 (ROI x Type)
fprintf('====================== RMAOV2_mod =========================\n');
fprintf('%.1f--%.1f s, %.1f--%.1f Hz: IV1: %s (%d;%s), IV2: %s (%d;%s), collapsing over %s (%d;%s)\n',...
  cfg_ana.latency(1),cfg_ana.latency(2),...
  cfg_ana.frequency(1),cfg_ana.frequency(2),...
  cfg_ana.IV_names{1},length(cfg_ana.roi),cfg_ana.chan_str,...
  cfg_ana.IV_names{2},length(cfg_ana.types),sprintf(repmat(' %s',1,length(cfg_ana.types)),cfg_ana.types{:}),...
  ...
  for j = 1:size(tt_combos1,1)
    ...
    fprintf('ttest: %s: %s (M=%.3f, SEM=%.3f) vs\t%s (M=%.3f, SEM=%.3f): t(%d)=%.4f, d=%.3f, SD=%.2f, SEM=%.2f, p=%.10f',...
      ...
      cfg_ana.types{tt_combos1(j,2)},mean(cond2,1),std(cond2)/sqrt(length(cond2)),...
        ...
        (cfg_ana.tstats.tstat/sqrt(cfg_ana.tstats.df)),... % cohen's d
        cfg_ana.tstats.sd,...
...
% IV1 (ROI)
fprintf('====================== RMAOV1_mod =========================\n');
fprintf('%.1f--%.1f s, %.1f--%.1f Hz: IV1: %s (%d;%s), collapsing over %s (%d;%s) and %s (%d;%s)\n',...
  cfg_ana.latency(1),cfg_ana.latency(2),...
  cfg_ana.frequency(1),cfg_ana.frequency(2),...
  cfg_ana.IV_names{1},length(cfg_ana.roi),cfg_ana.chan_str,...
  cfg_ana.IV_names{2},length(cfg_ana.types),sprintf(repmat(' %s',1,length(cfg_ana.types)),cfg_ana.types{:}),...
...
for i = 1:size(tt_combos,1)
  ...
  fprintf('ttest: %s (M=%.3f, SEM=%.3f) vs\t%s (M=%.3f, SEM=%.3f): t(%d)=%.4f, d=%.3f, SD=%.2f, SEM=%.2f, p=%.10f',...
    cfg_ana.roi{tt_combos(i,1)},mean(cfg_ana.X(cfg_ana.X(:,2) == tt_combos(i,1),1),1),std(cfg_ana.X(cfg_ana.X(:,2) == tt_combos(i,1),1))/length(cfg_ana.X(cfg_ana.X(:,2) == tt_combos(i,1),1)),...
    cfg_ana.roi{tt_combos(i,2)},mean(cfg_ana.X(cfg_ana.X(:,2) == tt_combos(i,2),1),1),std(cfg_ana.X(cfg_ana.X(:,2) == tt_combos(i,2),1))/length(cfg_ana.X(cfg_ana.X(:,2) == tt_combos(i,2),1)),...
      cfg_ana.tstats.df,...
      cfg_ana.tstats.tstat,...
      (cfg_ana.tstats.tstat/sqrt(cfg_ana.tstats.df)),... % cohen's d
      cfg_ana.tstats.sd,...
      (cfg_ana.tstats.sd/sqrt(length(cfg_ana.X(cfg_ana.X(:,2) == tt_combos(i,1),1)))),...
      cfg_ana.tp);
  if cfg_ana.tp < cfg_ana.alpha
    fprintf(' *');
  end
...
for i = 1:size(tt_combos,1)
  [cfg_ana.th,cfg_ana.tp,cfg_ana.tci,cfg_ana.tstats] = ttest(cfg_ana.X(cfg_ana.X(:,2) == tt_combos(i,1),1),cfg_ana.X(cfg_ana.X(:,2) == tt_combos(i,2),1),.05,'both');
  fprintf('ttest: %s (M=%.3f, SEM=%.3f) vs\t%s (M=%.3f, SEM=%.3f): t(%d)=%.4f, d=%.3f, SD=%.2f, SEM=%.2f, p=%.10f',...
    cfg_ana.types{tt_combos(i,1)},mean(cfg_ana.X(cfg_ana.X(:,2) == tt_combos(i,1),1),1),std(cfg_ana.X(cfg_ana.X(:,2) == tt_combos(i,1),1))/length(cfg_ana.X(cfg_ana.X(:,2) == tt_combos(i,1),1)),...
    cfg_ana.types{tt_combos(i,2)},mean(cfg_ana.X(cfg_ana.X(:,2) == tt_combos(i,2),1),1),std(cfg_ana.X(cfg_ana.X(:,2) == tt_combos(i,2),1))/length(cfg_ana.X(cfg_ana.X(:,2) == tt_combos(i,2),1)),...
      cfg_ana.tstats.df,...
      cfg_ana.tstats.tstat,...
      (cfg_ana.tstats.tstat/sqrt(cfg_ana.tstats.df)),... % cohen's d
      cfg_ana.tstats.sd,...
...
for i = 1:size(tt_combos,1)
  [cfg_ana.th,cfg_ana.tp,cfg_ana.tci,cfg_ana.tstats] = ttest(cfg_ana.X(cfg_ana.X(:,2) == tt_combos(i,1),1),cfg_ana.X(cfg_ana.X(:,2) == tt_combos(i,2),1),.05,'both');
  fprintf('ttest: %s (M=%.3f, SEM=%.3f) vs\t%s (M=%.3f, SEM=%.3f): t(%d)=%.4f, d=%.3f, SD=%.2f, SEM=%.2f, p=%.10f',...
    cfg_ana.condCommon{tt_combos(i,1)},mean(cfg_ana.X(cfg_ana.X(:,2) == tt_combos(i,1),1),1),std(cfg_ana.X(cfg_ana.X(:,2) == tt_combos(i,1),1))/length(cfg_ana.X(cfg_ana.X(:,2) == tt_combos(i,1),1)),...
    cfg_ana.condCommon{tt_combos(i,2)},mean(cfg_ana.X(cfg_ana.X(:,2) == tt_combos(i,2),1),1),std(cfg_ana.X(cfg_ana.X(:,2) == tt_combos(i,2),1))/length(cfg_ana.X(cfg_ana.X(:,2) == tt_combos(i,2),1)),...
      cfg_ana.tstats.df,...
      cfg_ana.tstats.tstat,...
      (cfg_ana.tstats.tstat/sqrt(cfg_ana.tstats.df)),... % cohen's d
      cfg_ana.tstats.sd,...
