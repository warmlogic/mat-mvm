...
for i = 1:length(levels1)
  for j = 1:size(tt_combos1,1)
    ...
    [cfg_ana.th,cfg_ana.tp,cfg_ana.tci,cfg_ana.tstats] = ttest(cond1,cond2,.05,'both');
    fprintf('ttest: %s: %s\t(M=%.3f) vs %s\t(M=%.3f): t(%d)=%.4f, p=%.10f',cfg_ana.roi{levels1(i,1)},cfg_ana.types{tt_combos1(j,1)},mean(cond1,1),cfg_ana.types{tt_combos1(j,2)},mean(cond2,1),cfg_ana.tstats.df,cfg_ana.tstats.tstat,cfg_ana.tp);
...
for i = 1:length(levels1)
  for j = 1:size(tt_combos1,1)
    ...
    [cfg_ana.th,cfg_ana.tp,cfg_ana.tci,cfg_ana.tstats] = ttest(cond1,cond2,.05,'both');
    fprintf('ttest: %s: %s\t(M=%.3f) vs %s\t(M=%.3f): t(%d)=%.4f, p=%.10f',cfg_ana.types{levels1(i,1)},cfg_ana.roi{tt_combos1(j,1)},mean(cond1,1),cfg_ana.roi{tt_combos1(j,2)},mean(cond2,1),cfg_ana.tstats.df,cfg_ana.tstats.tstat,cfg_ana.tp);
    if cfg_ana.tp < cfg_ana.alpha
      fprintf(' *');
    end
    fprintf('\n');
...
for i = 1:length(levels1)
  for j = 1:size(tt_combos1,1)
    ...
    cond2 = cfg_ana.X(cfg_ana.X(:,2) == levels1(i) & cfg_ana.X(:,3) == tt_combos1(j,2),1);
    [cfg_ana.th,cfg_ana.tp,cfg_ana.tci,cfg_ana.tstats] = ttest(cond1,cond2,.05,'both');
    fprintf('ttest: %s: %s\t(M=%.3f) vs %s\t(M=%.3f): t(%d)=%.4f, p=%.10f',cfg_ana.roi{levels1(i,1)},cfg_ana.condCommon{tt_combos1(j,1)},mean(cond1,1),cfg_ana.condCommon{tt_combos1(j,2)},mean(cond2,1),cfg_ana.tstats.df,cfg_ana.tstats.tstat,cfg_ana.tp);
    if cfg_ana.tp < cfg_ana.alpha
      fprintf(' *');
    end
    fprintf('\n');
...
for i = 1:length(levels1)
  for j = 1:size(tt_combos1,1)
    ...
    cond2 = cfg_ana.X(cfg_ana.X(:,3) == levels1(i) & cfg_ana.X(:,2) == tt_combos1(j,2),1);
    [cfg_ana.th,cfg_ana.tp,cfg_ana.tci,cfg_ana.tstats] = ttest(cond1,cond2,.05,'both');
    fprintf('ttest: %s: %s\t(M=%.3f) vs %s\t(M=%.3f): t(%d)=%.4f, p=%.10f',cfg_ana.condCommon{levels1(i,1)},cfg_ana.roi{tt_combos1(j,1)},mean(cond1,1),cfg_ana.roi{tt_combos1(j,2)},mean(cond2,1),cfg_ana.tstats.df,cfg_ana.tstats.tstat,cfg_ana.tp);
    if cfg_ana.tp < cfg_ana.alpha
      fprintf(' *');
    end
    fprintf('\n');
...
for i = 1:length(levels1)
  for j = 1:size(tt_combos1,1)
    ...
    cond2 = cfg_ana.X(cfg_ana.X(:,2) == levels1(i) & cfg_ana.X(:,3) == tt_combos1(j,2),1);
    [cfg_ana.th,cfg_ana.tp,cfg_ana.tci,cfg_ana.tstats] = ttest(cond1,cond2,.05,'both');
    fprintf('ttest: %s: %s\t(M=%.3f) vs %s\t(M=%.3f): t(%d)=%.4f, p=%.10f',cfg_ana.types{levels1(i,1)},cfg_ana.condCommon{tt_combos1(j,1)},mean(cond1,1),cfg_ana.condCommon{tt_combos1(j,2)},mean(cond2,1),cfg_ana.tstats.df,cfg_ana.tstats.tstat,cfg_ana.tp);
    if cfg_ana.tp < cfg_ana.alpha
      fprintf(' *');
    end
    fprintf('\n');
...
for i = 1:length(levels1)
  for j = 1:size(tt_combos1,1)
    ...
    cond2 = cfg_ana.X(cfg_ana.X(:,3) == levels1(i) & cfg_ana.X(:,2) == tt_combos1(j,2),1);
    [cfg_ana.th,cfg_ana.tp,cfg_ana.tci,cfg_ana.tstats] = ttest(cond1,cond2,.05,'both');
    fprintf('ttest: %s: %s\t(M=%.3f) vs %s\t(M=%.3f): t(%d)=%.4f, p=%.10f',cfg_ana.condCommon{levels1(i,1)},cfg_ana.types{tt_combos1(j,1)},mean(cond1,1),cfg_ana.types{tt_combos1(j,2)},mean(cond2,1),cfg_ana.tstats.df,cfg_ana.tstats.tstat,cfg_ana.tp);
    if cfg_ana.tp < cfg_ana.alpha
      fprintf(' *');
    end
    fprintf('\n');
...
for i = 1:size(tt_combos,1)
  [cfg_ana.th,cfg_ana.tp,cfg_ana.tci,cfg_ana.tstats] = ttest(cfg_ana.X(cfg_ana.X(:,2) == tt_combos(i,1),1),cfg_ana.X(cfg_ana.X(:,2) == tt_combos(i,2),1),.05,'both');
  fprintf('ttest: %s\t(M=%.3f) vs %s\t(M=%.3f): t(%d)=%.4f, p=%.10f',cfg_ana.roi{tt_combos(i,1)},mean(cfg_ana.X(cfg_ana.X(:,2) == tt_combos(i,1),1),1),cfg_ana.roi{tt_combos(i,2)},mean(cfg_ana.X(cfg_ana.X(:,2) == tt_combos(i,2),1),1),cfg_ana.tstats.df,cfg_ana.tstats.tstat,cfg_ana.tp);
  if cfg_ana.tp < cfg_ana.alpha
    fprintf(' *');
  end
  fprintf('\n');
...
for i = 1:size(tt_combos,1)
  [cfg_ana.th,cfg_ana.tp,cfg_ana.tci,cfg_ana.tstats] = ttest(cfg_ana.X(cfg_ana.X(:,2) == tt_combos(i,1),1),cfg_ana.X(cfg_ana.X(:,2) == tt_combos(i,2),1),.05,'both');
  fprintf('ttest: %s\t(M=%.3f) vs %s\t(M=%.3f): t(%d)=%.4f, p=%.10f',cfg_ana.types{tt_combos(i,1)},mean(cfg_ana.X(cfg_ana.X(:,2) == tt_combos(i,1),1),1),cfg_ana.types{tt_combos(i,2)},mean(cfg_ana.X(cfg_ana.X(:,2) == tt_combos(i,2),1),1),cfg_ana.tstats.df,cfg_ana.tstats.tstat,cfg_ana.tp);
  if cfg_ana.tp < cfg_ana.alpha
    fprintf(' *');
  end
  fprintf('\n');
...
for i = 1:size(tt_combos,1)
  [cfg_ana.th,cfg_ana.tp,cfg_ana.tci,cfg_ana.tstats] = ttest(cfg_ana.X(cfg_ana.X(:,2) == tt_combos(i,1),1),cfg_ana.X(cfg_ana.X(:,2) == tt_combos(i,2),1),.05,'both');
  fprintf('ttest: %s\t(M=%.3f) vs %s\t(M=%.3f): t(%d)=%.4f, p=%.10f',cfg_ana.condCommon{tt_combos(i,1)},mean(cfg_ana.X(cfg_ana.X(:,2) == tt_combos(i,1),1),1),cfg_ana.condCommon{tt_combos(i,2)},mean(cfg_ana.X(cfg_ana.X(:,2) == tt_combos(i,2),1),1),cfg_ana.tstats.df,cfg_ana.tstats.tstat,cfg_ana.tp);
  if cfg_ana.tp < cfg_ana.alpha
    fprintf(' *');
  end
  fprintf('\n');
