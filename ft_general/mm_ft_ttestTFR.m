...
%fprintf('I do not know if the calculation of cohen''s d is correct!\n');
...
if cfg_plot.line_plots == 1
  ...
  publishfig(gcf,0);
  if files.saveFigs
    cfg_plot.figfilename = sprintf('tfr_line_ga_%s%s%d_%d_%d_%d.%s',sprintf(repmat('%s_',1,length(cfg_plot.cond)),cfg_plot.cond{:}),cfg_plot.chan_str,cfg_ft.frequency(1),cfg_ft.frequency(2),cfg_ft.latency(1)*1000,cfg_ft.latency(2)*1000,files.figFileExt);
