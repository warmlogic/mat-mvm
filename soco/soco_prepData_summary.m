...
% % behavioral
% subjects = {...
%   'SOCO001'... % 90 targets, 1500 ms
%   'SOCO002'... % ended early by 6? trials
%   'SOCO003'... % 90 targets, 1500 ms
%   'SOCO004'... % 80 targets, 1500 ms
%   'SOCO005'... % 80 targets, 1500 ms
%   'SOCO006'... % 100 targets, 1000 ms
%   'SOCO007'...
%   'SOCO008'...
%   'SOCO009'...
%   'SOCO010'...
...
for s = 1:length(subsets)
  ...
  for sub = 1:length(subjects)
    ...
    for ses = 1:length(sessions)
      ...
      color_names = unique(getStructField(events,'study_color'));
      color_names = color_names(~cellfun('isempty',color_names));
      
      %empty = filterStruct(events,'ismember(study_color,varargin{1}) & ismember(type,varargin{2})','','STUDY_TARGET');

      % include events with defined study color name and that was tested
      events = filterStruct(events,'ismember(study_color,varargin{1}) & src_correct ~= -1',color_names);
      
      color_names2 = unique(getStructField(filterStruct(events,'numColors == 2',color_names),'study_color'));
      color_names6 = unique(getStructField(filterStruct(events,'numColors == 6',color_names),'study_color'));
      
%       for i = 1:length(color_names)
%         numcolev = length(filterStruct(events,'ismember(study_color,varargin{1}) & ismember(type,varargin{2})',color_names{i},'STUDY_TARGET'));
...
%         study_rh_srcInc = filterStruct(study_rh,'src_correct == 0');
        ...
        % explained in soco_createEvents.m
