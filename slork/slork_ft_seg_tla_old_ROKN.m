...
saveDirFigs = fullfile(saveDir,'figs_rokn');
...
for sub = 1:length(subjects)
  ...
  if ~exist(dataSaveFile,'file')
    ...
    data_preproc = seg2ft_nsmat(fullfile(dataroot,'ns_mat_rkon'),subjects{sub},prepost,sampleRate,elecfile);
...
num_RO = zeros(length(subjects),1);
...
tlaFilename = sprintf('data_RO_tla_%d_%d.mat',prepost(1)*1000,prepost(2)*1000);
tlaSaveFile = fullfile(saveDir,tlaFilename);
save(tlaSaveFile,'data_RO','num_RO');
...
data_RO = rmfield(data_RO,'dof');
data_RS = rmfield(data_RS,'dof');

%% decide who to kick out based on trial counts
[num_CR num_KNOW num_RO num_RS]
...
% 'SLORK003'    'SLORK009'    'SLORK013'    'SLORK014'    'SLORK015'
% 'SLORK017'    'SLORK018'    'SLORK023'    'SLORK029'
...
ro_mean = mean(num_RO(~ismember(subjects,badSub)));
ro_sd = std(num_RO(~ismember(subjects,badSub)),0,1);
ro_sem = std(num_RO(~ismember(subjects,badSub)),0,1)/sqrt(sum(~ismember(subjects,badSub)));
fprintf('RO: M=%.3f, SD=%.3f, SEM=%.3f\n',ro_mean,ro_sd,ro_sem);
...
eval(sprintf('ga_RO = ft_timelockgrandaverage(cfg,%s);',str_RO));
...
figure
ft_singleplotER(cfg,ga_RS,ga_RO,ga_KNOW,ga_CR);
legend('RSrc','RO','F','N','Location','SouthEast');
% P O/N
cfg.ylim = [-2 5];
cfg.channel = cat(2,elecGroups{ismember(elecGroupsStr,{'LPS','RPS'})});
figure
ft_singleplotER(cfg,ga_RS,ga_RO,ga_KNOW,ga_CR);
legend('RSrc','RO','F','N','Location','NorthEast');
...
for sub = 1:length(subjects)
  ...
  hold on
  plot(data_RO(sub).time,mean(data_RO(sub).avg(chan,:),1),'g');
  ...
  if ismember(subjects{sub},badSub)
    title(sprintf('*BAD*: %s; N:%d, F:%d, RO:%d, RSrc:%d',subjects{sub},size(data_CR(sub).trial,1),size(data_KNOW(sub).trial,1),size(data_RO(sub).trial,1),size(data_RS(sub).trial,1)));
  else
    title(sprintf('%s; N:%d, F:%d, RO:%d, RSrc:%d',subjects{sub},size(data_CR(sub).trial,1),size(data_KNOW(sub).trial,1),size(data_RO(sub).trial,1),size(data_RS(sub).trial,1)));
  end
  %ylim([0 1.9e-13]);
  axis([-0.2 1.5 -10 10]);
  hold off
...
text(0.5,0.5,'RO','color','g')
text(0.5,0.3,'F','color','b') ;
text(0.5,0.1,'N','color','k')
axis off
...
% create contrast
ga_ROvsCR = ga_RO;
ga_ROvsCR.avg = ga_RO.avg - ga_CR.avg;
ga_ROvsCR.individual = ga_RO.individual - ga_CR.individual;
% make a plot
figure
roi = {'LAS','RAS'};
cond = {'RO','N'};
...
% create contrast
ga_RSvsRO = ga_RS;
ga_RSvsRO.avg = ga_RS.avg - ga_RO.avg;
ga_RSvsRO.individual = ga_RS.individual - ga_RO.individual;
% make a plot
figure
roi = {'LAS','RAS'};
cond = {'RSrc','RO'};
...
% create contrast
ga_ROvsKNOW = ga_RO;
ga_ROvsKNOW.avg = ga_RO.avg - ga_KNOW.avg;
ga_ROvsKNOW.individual = ga_RO.individual - ga_KNOW.individual;
% make a plot
figure
roi = {'LAS','RAS'};
cond = {'RO','F'};
...
timesel_RO = find(ga_RO.time >= cfg.latency(1) & ga_RO.time <= cfg.latency(2));
...
values_RO = mean(mean(ga_RO.individual(:,chan,timesel_RO),2),3);
...
sem_RO =  std(values_RO)/sqrt(length(values_RO));
sem_RS =  std(values_RS)/sqrt(length(values_RS));

fprintf('\t%s\t%s\t%s\t%s\n','N','F','RO','RSrc');
% ga
[mean(values_CR,1) mean(values_KNOW,1) mean(values_RO,1) mean(values_RS,1)]
% sub avg
...
stat_ttest_RSvsRO = ft_timelockstatistics(cfg,ga_RS,ga_RO);
stat_ttest_ROvsKNOW = ft_timelockstatistics(cfg,ga_RO,ga_KNOW);
stat_ttest_ROvsCR = ft_timelockstatistics(cfg,ga_RO,ga_CR);

% side
fprintf('RSrc (M=%.3f; SEM=%.3f) vs F (M=%.3f; SEM=%.3f):\tt(%d)=%.4f, p=%.10f\n',mean(values_RS,1),sem_RS,mean(values_KNOW,1),sem_KNOW,stat_ttest_RSvsKNOW.df,stat_ttest_RSvsKNOW.stat,stat_ttest_RSvsKNOW.prob);
fprintf('F (M=%.3f; SEM=%.3f) vs N  (M=%.3f; SEM=%.3f):\tt(%d)=%.4f, p=%.10f\n',mean(values_KNOW,1),sem_KNOW,mean(values_CR,1),sem_CR,stat_ttest_KNOWvsCR.df,stat_ttest_KNOWvsCR.stat,stat_ttest_KNOWvsCR.prob);
fprintf('RSrc (M=%.3f; SEM=%.3f) vs N  (M=%.3f; SEM=%.3f):\tt(%d)=%.4f, p=%.10f\n',mean(values_RS,1),sem_RS,mean(values_CR,1),sem_CR,stat_ttest_RSvsCR.df,stat_ttest_RSvsCR.stat,stat_ttest_RSvsCR.prob);
fprintf('RSrc (M=%.3f; SEM=%.3f) vs RO  (M=%.3f; SEM=%.3f):\tt(%d)=%.4f, p=%.10f\n',mean(values_RS,1),sem_RS,mean(values_RO,1),sem_RO,stat_ttest_RSvsRO.df,stat_ttest_RSvsRO.stat,stat_ttest_RSvsRO.prob);
fprintf('RO (M=%.3f; SEM=%.3f) vs F (M=%.3f; SEM=%.3f):\tt(%d)=%.4f, p=%.10f\n',mean(values_RO,1),sem_RO,mean(values_KNOW,1),sem_KNOW,stat_ttest_ROvsKNOW.df,stat_ttest_ROvsKNOW.stat,stat_ttest_ROvsKNOW.prob);
fprintf('RO (M=%.3f; SEM=%.3f) vs N  (M=%.3f; SEM=%.3f):\tt(%d)=%.4f, p=%.10f\n',mean(values_RO,1),sem_RO,mean(values_CR,1),sem_CR,stat_ttest_ROvsCR.df,stat_ttest_ROvsCR.stat,stat_ttest_ROvsCR.prob);
fprintf('\n');
...
for i = 1:length(conditions)
  ...
  if length(cond) == 4
    % lines
    plot([mean(values_RS,1),mean(values_RO,1),mean(values_KNOW,1),mean(values_CR,1)],cfg_plot.side.linespec,'LineWidth',cfg_plot.linewidth);
    hold on
    % errorbars
    ...
    h = errorbar(2,mean(values_RO,1),sem_RO,cfg_plot.side.linespec,'LineWidth',cfg_plot.errwidth);
    % remove errorbar ends
    if cfg_plot.removeErrBarEnds
      c = get(h,'Children');
      xdata = get(c(2),'XData');
    ...
    h = errorbar(3,mean(values_KNOW,1),sem_KNOW,cfg_plot.side.linespec,'LineWidth',cfg_plot.errwidth);
    % remove errorbar ends
    if cfg_plot.removeErrBarEnds
      c = get(h,'Children');
      xdata = get(c(2),'XData');
    ...
    h = errorbar(4,mean(values_CR,1),sem_CR,cfg_plot.side.linespec,'LineWidth',cfg_plot.errwidth);
    % remove errorbar ends
    if cfg_plot.removeErrBarEnds
      c = get(h,'Children');
      xdata = get(c(2),'XData');
    ...
    plot(2,mean(values_RO,1),cfg_plot.side.linespec,'LineWidth',cfg_plot.linewidth,'MarkerSize',cfg_plot.marksize,'MarkerFaceColor',cfg_plot.side.markcolor);
    plot(3,mean(values_KNOW,1),cfg_plot.side.linespec,'LineWidth',cfg_plot.linewidth,'MarkerSize',cfg_plot.marksize,'MarkerFaceColor',cfg_plot.side.markcolor);
    plot(4,mean(values_CR,1),cfg_plot.side.linespec,'LineWidth',cfg_plot.linewidth,'MarkerSize',cfg_plot.marksize,'MarkerFaceColor',cfg_plot.side.markcolor);
  end
  % make it look good
...
stat_clus_RSvsRO = ft_timelockstatistics(cfg,ga_RS,ga_RO);
stat_clus_RSvsCR = ft_timelockstatistics(cfg,ga_RS,ga_CR);
stat_clus_ROvsCR = ft_timelockstatistics(cfg,ga_RO,ga_CR);
stat_clus_ROvsKNOW = ft_timelockstatistics(cfg,ga_RO,ga_KNOW);
...
ft_clusterplot(cfg,stat_clus_RSvsRO); % none
ft_clusterplot(cfg,stat_clus_RSvsCR); % pos front, neg back
ft_clusterplot(cfg,stat_clus_ROvsCR); % pos front, neg back
ft_clusterplot(cfg,stat_clus_ROvsKNOW); % none
