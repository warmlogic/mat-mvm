function slork_events2ns(dataroot,subject,session)
% slork_events2ns(dataroot,subject,session)
%
...
for i = 1:length(events)
  ...
  trialStr = 'tril';
  % item info
  testStr = 'test'; % type of test: task or side
  typeStr = 'type'; % type of response (STUDY_RESP, SOURCE_RESP, etc.)
  itemStr = 'item'; % item name
  poolStr = 'pool'; % item's pool
  pcorStr = 'pcor'; % was the pool correct?
  xcrdStr = 'xcrd'; % xcoord
  ycrdStr = 'ycrd'; % ycoord
  srpsStr = 'srps'; % serial position
  % study resp
  scndStr = 'scnd'; % the study condition
  sturStr = 'stur'; % the response
  strtStr = 'strt'; % reaction time
  ...
  % item info
  task = events(i).testType;
  type = events(i).type;
  item = events(i).item;
  pool = events(i).pool;
  pool_correct = num2str(events(i).pool_correct);
  xcoord = num2str(events(i).xcoord);
  ycoord = num2str(events(i).ycoord);
  serpos = num2str(events(i).serialpos);
  
  % study
  study_cond = events(i).study_cond;
  ...
  %cellLabels = {subStr,sesStr,listStr,trialStr,testStr,typeStr,itemStr,poolStr,pcorStr,xcrdStr,ycrdStr,srpsStr,rec_targStr,scndStr,sturStr,strtStr,src_targStr,srcrStr,srrtStr,scorStr,rknrStr,rkrtStr,rcorStr,artiStr};
  %cellData = {subject,session,list,trial,task,type,item,pool,pool_correct,xcoord,ycoord,serpos,rec_isTarg,study_cond,study_resp,study_rt,src_isTarg,src_resp,src_rt,src_correct,rkn_resp,rkn_rt,rkn_correct,artifactMS};
  cellLabels = {subStr,sesStr,listStr,trialStr,testStr,typeStr,itemStr,poolStr,pcorStr,xcrdStr,ycrdStr,srpsStr,scndStr,sturStr,strtStr,rec_targStr,rcorStr,src_targStr,srcrStr,srrtStr,scorStr,rknrStr,rkrtStr,rkncStr};
  cellData = {subject,session,list,trial,task,type,item,pool,pool_correct,xcoord,ycoord,serpos,study_cond,study_resp,study_rt,rec_isTarg,rec_correct,src_isTarg,src_resp,src_rt,src_correct,rkn_resp,rkn_rt,rkn_correct};
